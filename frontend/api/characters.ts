...
export async function getCharacterDetail(slug: string): Promise<CharacterDetail> {
  const res = await fetch(`${process.env.BACKEND_URL}/api/characters/${slug}`, { cache: "no-store" });
...
export async function getAllCharacters(): Promise<CharacterListItem[]> {
  const res = await fetch(`${process.env.BACKEND_URL}/api/characters`, { cache: "no-store" });
