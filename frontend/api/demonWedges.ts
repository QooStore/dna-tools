...
export async function getAllDemonWedges(): Promise<DemonWedgeListItem[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/demon-wedges`, { cache: "no-store" });
...
export async function getDemonWedgeDetail(slug: string): Promise<DemonWedgeDetail> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/demon-wedges/${slug}`, { cache: "no-store" });
